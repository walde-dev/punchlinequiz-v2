...
import { getProfileSnapshot, levelFor, levelInfos, loadLevels } from "./xp"
...
export const getProfileFn = createServerFn({ method: "GET" }).handler(
  async (): Promise<ProfileFnResult> => {
    const req = getRequest()
    const result = await getActor(req)
    if (result?.actor.kind === "clerk") {
      return {
        signedIn: true,
        profile: await getProfileSnapshot(result.actor.userId),
      }
    }
...
export const getHeaderXpFn = createServerFn({ method: "GET" }).handler(
  async (): Promise<HeaderXp> => {
    const req = getRequest()
    const result = await getActor(req)
    const sorted = await loadLevels()
    if (result?.actor.kind !== "clerk") {
