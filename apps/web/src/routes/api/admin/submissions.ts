...
import { getTiersFor } from "../../../lib/contributor"
import type { ContributorTier } from "../../../lib/contributor"

/** Higher = reviewed first. Trusted contributors jump the pending queue (PUN-66). */
const TIER_PRIORITY: Record<ContributorTier, number> = {
  verifiziert: 2,
  vertraut: 1,
  neuling: 0,
}
      ...
      GET: async ({ request }) => {
        try {
          ...
          const rows = await db
            ...
            .innerJoin(
              users,
              eq(users.clerkId, punchlineSubmissions.submitterClerkId)
            )
          ...
          const tiers = await getTiersFor([
            ...new Set(rows.map((r) => r.submitterClerkId)),
          ])
          let items = rows.map(({ submitterClerkId, ...r }) => ({
            ...r,
            submitterTier:
              tiers.get(submitterClerkId) ?? ("neuling" as ContributorTier),
          }))
          if (status === "pending") {
            items = items.sort((a, b) => {
              const d =
                TIER_PRIORITY[b.submitterTier] - TIER_PRIORITY[a.submitterTier]
              if (d !== 0) return d
              return (
