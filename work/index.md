# work index

auto-generated. last updated: 2026-05-13

## features

| priority | title | status | tags |
|---|---|---|---|
| **p0** | [Share cards — the viral engine](features/p0-share-cards.md) | `scoped` | viral, distribution, core |
| **p1** | [Artist selection mode](features/p1-artist-selection.md) | `scoped` | distribution, gameplay, core |
| **p1** | [Daily challenge](features/p1-daily-challenge.md) | `idea` | retention, viral, gameplay |
| **p2** | [Leaderboard](features/p2-leaderboard.md) | `scoped` | retention, competitive, gameplay |
| **p3** | [XP / progression system](features/p3-xp-system.md) | `idea` | retention, gamification |
| **p3** | [More game modes](features/p3-more-game-modes.md) | `idea` | gameplay, retention |

## ideas

_empty — no ideas filed yet._

## bugs

_empty — no bugs filed yet._

## plans

_empty — no plans written yet._

## priority guide

- **p0** — blocks launch. do this now.
- **p1** — ship this week. high impact.
- **p2** — ship this month. important but not blocking.
- **p3** — nice to have. ship when there's bandwidth.

## quick commands (for agents)

```
# add a new work item
write_file("work/features/p1-my-feature.md", frontmatter + content)
# then regenerate index.md

# check current priorities
read_file("work/index.md")

# move idea → feature
# 1. copy content, update status to "scoped"
# 2. write to features/ with proper priority prefix
# 3. delete from ideas/
# 4. regenerate index.md
```
